...
void web_server_init()
{
  ...
  server.on("/update", HTTP_GET, [](AsyncWebServerRequest* request) {
    if (request->hasParam(HTTP_MOTOR_MODULE_Y) &&
        request->hasParam(HTTP_MOTOR_SPEED) &&
        request->hasParam(HTTP_MOTOR_X)) {
      machine_room_update(request->getParam(HTTP_MOTOR_MODULE_Y)->value().toInt(),
                          request->getParam(HTTP_MOTOR_SPEED)->value().toInt(),
